var handleState, position, withTransliteration, selectedLetterOrigin, selectedLetterTransliteration, getOrigin, getTransliteration, allVowels, allNonVowels, allConsonants, allSemiVowels, allSibilants, allOthers, allLetters, presentedWord, presentedWordIndex, getWords;

function generalStartup() {
  $("#view-options").buttonset();
  $("#extra-options").buttonset();
  $("#view-options input, #extra-options input").click(function() {showWord();});
  ...
  $("#navigate-next").button().click(function() {
    if (presentedWordIndex === undefined || presentedWordIndex === "word-" + ($("#word-list li").length - 1)) {
  ...
  $("#navigate-prev").button().click(function() {
  ...
  if ($("#view-options #devanagari, #view-options #devanagari-with-iast").length > 0) {
    $("#view-options #devanagari, #view-options #devanagari-with-iast").get(0).checked = true;
  }
  $("#view-options #devanagari, #view-options #devanagari-with-iast").button("refresh");
  $("#view-options, #show-state").keyup(preventDefaultEvent);
  $(document).keydown(function(event) {
     if (event.which === " ".charCodeAt()) {if (!withTransliteration) showHint()};
  });
  $(window).resize(orderInFives);
  $("#guidance").click(hideGuidance);
}

function changeAndShowWord(direction, wordID) {
...
function hideGuidance() {
  if ($("#guidance").is(":visible")) {
    $( "#guidance" ).hide("slide", function() {
      $("<button id='guidance-handle'>Show Hint</button>").button().click(function() {
        $(this).hide();
        $("#guidance").show("slide");
...
function onKeyDoNotShowLetter() {
  $(document).unbind("keyup");
}
function onKeyShowLetter() {
  $(document).keyup(function(event) {
     var newKey = NaN;
     if (event.which === " ".charCodeAt()) {if (!withTransliteration) hideHint()};
     if (event.which === "1".charCodeAt()) {newKey=1;}
     if (event.which === "2".charCodeAt()) {newKey=2;}
     if (event.which === "3".charCodeAt()) {newKey=3;}
     if (event.which === "4".charCodeAt()) {newKey=4;}
     if (event.which === "5".charCodeAt()) {newKey=5;}
     if (event.which === "6".charCodeAt()) {newKey=6;}
     if (event.which === "7".charCodeAt()) {newKey=7;}
     if (event.which === "8".charCodeAt()) {newKey=8;}
     if (event.which === "9".charCodeAt()) {newKey=9;}
     if (event.which === "0".charCodeAt()) {newKey=10;}
     if (event.which === "Q".charCodeAt()) {newKey=11;}
     if (event.which === "W".charCodeAt()) {newKey=12;}
     if (event.which === "E".charCodeAt()) {newKey=13;}
     if (event.which === "R".charCodeAt()) {newKey=14;}
     if (event.which === "T".charCodeAt()) {newKey=15;}
     if (event.which === "Y".charCodeAt()) {newKey=16;}
     if (event.which === "U".charCodeAt()) {newKey=17;}
     if (event.which === "I".charCodeAt()) {newKey=18;}
     if (event.which === "O".charCodeAt()) {newKey=19;}
     if (event.which === "P".charCodeAt()) {newKey=20;}
     if (event.which === "A".charCodeAt()) {newKey=21;}
     if (event.which === "S".charCodeAt()) {newKey=22;}
     if (event.which === "D".charCodeAt()) {newKey=23;}
     if (event.which === "F".charCodeAt()) {newKey=24;}
     if (event.which === "G".charCodeAt()) {newKey=25;}
     if (event.which === "H".charCodeAt()) {newKey=26;}
     if (event.which === "J".charCodeAt()) {newKey=27;}
     if (event.which === "K".charCodeAt()) {newKey=28;}
     if (event.which === "L".charCodeAt()) {newKey=29;}
     if (event.which === "Z".charCodeAt()) {newKey=30;}
     if (event.which === "X".charCodeAt()) {newKey=31;}
     if (event.which === "C".charCodeAt()) {newKey=32;}
     if (event.which === "V".charCodeAt()) {newKey=33;}
     if (event.which === "B".charCodeAt()) {newKey=34;}
     if (event.which === "N".charCodeAt()) {newKey=35;}
     if (event.which === "M".charCodeAt()) {newKey=36;}
     if (!isNaN(newKey)) {
...
function getAspirateAndCompoundsAndAvagraha() {
...
function getSyllablesIast(source) {
  var syllables = [];
  ...
  while (letter !== "") {
    if (!isLetter(letter) || (handleState === handleState0 && letter !== "ḥ" && letter != "ṃ")) {
      currentSyllable++;
...
function getNextLetter(source) {
  var letter =  source.charAt(position);
  if ((isVowel(letter) && isVowel(source.charAt(position+1)) && source.charAt(position+1) !== 'ṃ' && source.charAt(position+1) !== 'ḥ') || (isNonVowel(letter) && source.charAt(position+1) === 'h' && letter !== "'")) {
    position++;
...
function isLetter(letter) {
  ...
  for (var i=0; i<getAllLetters().length;i++) {
    if (Object.keys(getAllLetters()[i])[0] === letter || Object.keys(getAllLetters()[i])[0] === letter + "a") {
      return true;
    }
  }
  return false;
...
function showHint() {
  if ($("#individual-letter .transliteration").length === 0) {
    $("#individual-letter").append("<div class='transliteration'>" + (selectedLetterTransliteration === undefined ? "" : selectedLetterTransliteration) + "</div>");
  }
}
...
function showTheLetters() {
    ...
    if (viewOption === "iast-only") {
        getOrigin = function(key,val) {return key;};
        getTransliteration = function(key,val) {return val;};
        ...
        getOrigin = function(key,val) {return val;};
        getTransliteration = function(key,val) {return key;};
        withTransliteration = false;
        ...
        getOrigin = function(key,val) {return "";};
        getTransliteration = function(key,val) {return "";};
    }
...
function preventDefaultEvent(event) {
  if (selectedLetterOrigin !== "") {event.preventDefault();}
}

function getWordElements(source) {
  if (source === "ya") {
    var a=1;
  }
  var wordElements = [];
  ...
  while (letter !== "") {
    if (isLetter(letter)) {
      if (letter === "aḥ") {letter = "ḥ";}
      if (letter === "aṃ") {letter = "ṃ";}
...
function appendLetterInEquations(key,val,syllableCount) {
  var elements = getWordElements(key);
  if (key === "'") key = "avagraha";
  if (elements.length > 0) {
    $("<div>", {id:"syllable_" + syllableCount}).appendTo("#equations");
      $("div#syllable_"+syllableCount).prepend("<div class='transliteration-syllable'>" + key + "</div> → ");
    var j;
    for (var i=0;i<elements.length;i++) {
      $("#equations div#syllable_"+syllableCount).append("<div id='element_"+i+"' class='equation'>"+elements[i]+"</div><span class='plus'> + </span>");
      j=i;
    }
    $("#equations div#syllable_"+syllableCount+" .plus").last().remove();
    $("div#syllable_"+syllableCount).append(" = " + val);
  } else {
    $("<div>", {id:"syllable_"+syllableCount}).append(val).appendTo("#equations");
  }
}

function showTransliteration(viewOption) {
  var wordToShow = (viewOption !== "iast-only") ? presentedWord : transliterate(presentedWord);
  $("<div>").append(wordToShow).appendTo("#main-block #transliteration");
}

function showTranslation() {
  for (var i = 0; i < getWords().length; i++) {
    if (presentedWord === Object.keys(getWords()[i])[0]) {
      $("<div>").append(getWords()[i][presentedWord]).appendTo("#main-block #translation");
    }
  }
}

function showSyllables(viewOption) {
  var syllables = (viewOption === "iast") ? getSyllablesIast(presentedWord) : getSyllables(presentedWord);
  for (var i = 0; i < syllables.length; i++) {
    if (viewOption === "devanagari-only") {
       appendLetterInEquations(getSyllablesIast(presentedWord)[i],syllables[i],i);
    } else {
      $("<div>", {id:getSyllablesIast(presentedWord)[i]}).append(syllables[i]).appendTo("#main-block #equations");
    }
  }
}

function showWord(animate) {
  ...
  var wordToShow = (viewOption === "iast-only") ? presentedWord : transliterate(presentedWord);
  if (wordToShow === undefined) {wordToShow = "";}
